...
public class PlayListItem implements ITimeEventListener {
	...
	public PlayListItem(ISound sound, PlayerControllerSettings settings, PlayList parent) {
	 ...
	 */
	public PlayListItem(PlayListItem playListItem, boolean move) {
		...
		this.parent = playListItem.parent;
		
		if (move) {
			wrappers = playListItem.wrappers;
			this.status = playListItem.status;
			if (status != PlayerControllerStatus.STOPPED) {
				for (PlayerWrapper wrapper : wrappers) {
					if (wrapper.getStatus() != PlayerControllerStatus.STOPPED) {
	 ...
	 */
	public PlayListItem(PropertyMap map) throws LoadingException {
	 ...
	 */
	public void setSettings(PlayerControllerSettings settings) {
		this.settings = settings;
	}
	...
	/**
	 * Sets the status for this <c>PlayListItem</c>.
	...
	/**
	 * @return The <c>PlayListItem</c>'s status.
	 */
	...
	private void setCurrentVolume(PlayerWrapper pWrapper) {
		if (settings == null) {
			return;
		}
	...
	@Override
	public synchronized boolean receiveTimeEvent(long currentTime, Object obj) {
